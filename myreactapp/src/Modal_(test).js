...
function App() {
	...
	const openModal = (itemId) => {
		// 모달 열기
		setIsModalOpen(true);
		console.log('Open modal for item with ID:', itemId);
		setMenunum(itemId);
	};
										...
										.map((menuItem) => (
											...
											>
												...
												<div className="menu-card-body">
													...
													<div className="menu-price">{menuItem.price}원</div>
												</div>
											</div>
										))}
								</div>
				...
				<div className="order-container">
					{selectedItems.slice().reverse().map((item, index) => (
						...
						<div className="order-box" key={index}>
							<Modal isOpen={isModalOpen} onClose={closeModal} menuinfo={menunum}>
								<h2>${item.en}</h2>
							</Modal>
							...
							<div className="order-item-body">
								...
								<div className="order-item-body-right">
									...
									<div className="order-item-body-right-bottom">
										<div className="order-item-option" onClick={() => openModal(item.id)}>옵션변경</div>
